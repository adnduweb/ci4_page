$(document).ready(function() {
    ...
    $(".form_builder_area").sortable({ cancel: '.kt-portlet__content, .textarea_click' });
    //$(".form_builder_area").sortable({ items: '.kt-portlet__head' });
    ...
    function getTextFieldHTML() {
        var field = generateField();
        var html = $('.compoments_textfield').html();
        var re = /__field__/gi;
        html = html.replace(re, '__' + field);
        var script = '<script>jQuery(document).ready(function() {var portlet' + field + ' = new KTPortlet("kt_portlet_tools__' + field + '");  portlet' + field + '.collapse();}); </script>';
        html = html.replace('__script__', script);
        var portlet = new KTPortlet("kt_portlet_tools__" + field);
        return $('<div>').addClass('conpoment_build compoment_' + field + ' ').html(html);
    }

    function getImageFieldHTML() {
        var field = generateField();
        var html = $('.compoments_imagefield').html();
        var re = /__field__/gi;
        html = html.replace(re, '__' + field);

        //KTDropzoneOnlyImagesBuilder.init(field);
    ...
    function getTextAreaFieldHTML() {
        ...
        // return $('<div>').addClass('li_' + field + ' form_builder_field').html(html);
        var field = 2956771970398;
        var html = $('.compoments_textarea').html();
        var re = /__field__/gi;
        html = html.replace(re, '__' + field);
        var script = '<script>jQuery(document).ready(function() {var portlet' + field + ' = new KTPortlet("kt_portlet_tools__' + field + '"); portlet' + field + '.collapse();});</script>';
        if (activer_multilangue == 1) {
            var langues = supportedLocales.split(',');
            $.each(langues, function(key, value) {
                script += '<script>jQuery(document).ready(function() {ClassicEditor.create(document.querySelector("#content__' + field + '_' + value + '"), { language: "' + lang_iso + '" }).then(editor => { console.log(editor); }).catch(error => { console.error(error); });}); </script>';
            });
        } else {
            script += '<script>jQuery(document).ready(function() {ClassicEditor.create(document.querySelector("#content__' + field + '_' + lang_iso + '"), { language: "' + lang_iso + '" }).then(editor => { console.log(editor); }).catch(error => { console.error(error); });}); </script>';
        }
    ...
    $(document).on('click', '.removePortlet', function() {
        var id_builder = $(this).data('id_builder');
        if (id_builder != '') {
            $.ajax({
                type: 'POST',
    ...
    // $(document).on('click', 'a[data-ktportlet-tool="toggle"]', function() {
    //     console.log('fabrice');
    //     portlet.collapse();
    // });
