								...
								<div class="kt-form__body">
									<div class="kt-section kt-section--first">
										<div class="kt-section__body">
											<?= $this->include('\Adnduweb\Ci4_page\Views\admin\themes\metronic\__form_section\general') ?>
										</div>
									</div>
								</div>
							</div>
						...
						<div class="tab-pane" id="kt_user_edit_tab_2" role="tabpanel">
							<div class="kt-form kt-form--label-right">
								<div class="kt-form__body">
									<div class="kt-section kt-section--first">
										<div class="kt-section__body">
											<?= $this->include('/admin/themes/metronic/__partials/builder/builder') ?>
										</div>
									</div>
								</div>
							</div>
...
<?php if (isset($form->builders) && !empty($form->builders)) { ?>
	...
	foreach ($form->builders as $builder) { ?>
		<?php if ($builder->type == "textareafield") {
		...
		<?php } ?>
		<?php if ($builder->type == "textfield" || $builder->type == "imagefield" || $builder->type == "diaporamafield" || $builder->type == "actufield") {
			$field = isset($builder->id_field) ? $builder->id_field : "__field__"; ?>
			<script>
				jQuery(document).ready(function() {
					var portlet<?= $field; ?> = new KTPortlet("kt_portlet_tools<?= $field; ?>", optionsPortlet);
